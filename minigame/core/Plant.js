 ...
 * 植物实例 - 单棵植物的状态管理
 */
import { PlantType, HealthState, StressType, PLANT_CONFIGS } from './PlantTypes';
...
function healthValueToState(value) {
    ...
    if (value < 25)
        return HealthState.SEVERE;
    if (value < 50)
        return HealthState.DAMAGED;
    if (value < 75)
        return HealthState.MINOR_DAMAGE;
...
function getWeatherDescription(weather) {
    ...
    if (weather.precipitation > 0)
        return `🌦️ 小雨 ${temp}°C`;
    if (weather.sunlight > 0.8)
        return `☀️ 晴天 ${temp}°C`;
    if (weather.sunlight > 0.5)
        return `⛅ 多云 ${temp}°C`;
    return `☁️ 阴天 ${temp}°C`;
}
/**
