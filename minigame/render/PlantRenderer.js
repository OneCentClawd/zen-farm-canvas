 ...
 * 用纯 Canvas API 绘制植物
 */
import { PlantType, HealthState } from '../core/PlantTypes';
// 颜色配置
const COLORS = {
    stem: '#4c994c', // 茎秆绿
...
export class PlantRenderer {
     ...
     * 渲染植物
     */
    render(plant, x, y, scale = 1, deltaTime = 0) {
     ...
     * 绘制萌发根系
     */
    drawGerminatingRoots(plant, progress, seedY, seedHeight) {
        const ctx = this.ctx;
     ...
     * 绘制发芽阶段
     */
    drawSprout(plant, progress, wiltLevel) {
        const ctx = this.ctx;
        // 先画根系
     ...
     * 绘制完整植物
     */
    drawPlant(plant, progress, wiltLevel, isDead) {
        // 根据植物类型绘制
        switch (plant.type) {
     ...
     * 绘制根系
     */
    drawRoots(depth, spread, branchCount, structure, progress = 1) {
        const ctx = this.ctx;
        // 主根
        ctx.strokeStyle = COLORS.root;
        ctx.lineWidth = 2;
        ...
        if (structure) {
            for (let i = 0; i < Math.min(branchCount, structure.length); i++) {
                ...
                ctx.beginPath();
                ctx.moveTo(0, startY);
                ctx.quadraticCurveTo(Math.sin(angle) * length * 0.5, startY - length * 0.3, Math.sin(angle) * length, startY - length * 0.8);
                ctx.stroke();
            }
        }
    }
     ...
     * 绘制三叶草
     */
    drawClover(plant, progress, wiltLevel, isDead) {
        const ctx = this.ctx;
     ...
     * 绘制向日葵
     */
    drawSunflower(plant, progress, wiltLevel, isDead) {
        const ctx = this.ctx;
        const stemHeight = plant.height * 3;
     ...
     * 绘制草莓
     */
    drawStrawberry(plant, progress, wiltLevel, isDead) {
        const ctx = this.ctx;
        ...
        for (let i = 0; i < leafCount; i++) {
            ...
            ctx.beginPath();
            ctx.ellipse(Math.sin(angle) * leafSize, height * 0.3 + Math.cos(angle) * leafSize * 0.5, leafSize, leafSize * 0.5, angle, 0, Math.PI * 2);
            ctx.fill();
        }
        // 果实
     ...
     * 绘制樱花
     */
    drawSakura(plant, progress, wiltLevel, isDead) {
        const ctx = this.ctx;
        ...
        if (progress > 0.6 && !isDead) {
            ...
            for (let i = 0; i < flowerCount; i++) {
                ...
                for (let p = 0; p < 5; p++) {
                    ...
                    ctx.beginPath();
                    ctx.ellipse(fx + Math.cos(angle) * fsize * 0.5, fy + Math.sin(angle) * fsize * 0.5, fsize * 0.4, fsize * 0.25, angle, 0, Math.PI * 2);
                    ctx.fill();
                }
                // 花心
...
//# sourceMappingURL=PlantRenderer.js.map